                ...
                <div class="row">
                    <div class="col-xs-12 col-md-8">
                        <div class="top_row_new">
                            <div class='brd_cls'>
                                @if (isset($recordInfo->Category->slug))
                                    <a
                                        href="{{ URL::to('gigs/' . $recordInfo->Category->slug) }}">{{ $recordInfo->Category->name }}</a>
                                    @if (isset($recordInfo->Subcategory->slug))
                                        >

                                        <a
                                            href="{{ URL::to('gigs/' . $recordInfo->Category->slug . '/' . $recordInfo->Subcategory->slug) }}">{{ $recordInfo->Subcategory->name }}</a>
                                    @endif
                                @endif
                            ...
                            <div class="seller_review">
                                ...
                                <div class="sel_img">
                                    ...
                                    <div class="sel_img_img">
                                        ...
                                        @if (file_exists(PROFILE_FULL_UPLOAD_PATH . $recordInfo->User->profile_image) &&
                                                !empty($recordInfo->User->profile_image))
                                            <a
                                                href="{{ URL::to('public-profile/' . $recordInfo->User->slug) }}">{{ HTML::image(PROFILE_SMALL_DISPLAY_PATH . $recordInfo->User->profile_image, SITE_TITLE) }}</a>
                                        @else
                                            <a
                                                href="{{ URL::to('public-profile/' . $recordInfo->User->slug) }}">{{ HTML::image('public/img/front/dummy.png', SITE_TITLE) }}</a>
                                        @endif
                                    ...
                                    <div class="sel_img_txt">
                                        ...
                                        <a href="{{ URL::to('public-profile/' . $recordInfo->User->slug) }}">{{ $recordInfo->User->first_name ? $recordInfo->User->first_name . ' ' . $recordInfo->User->last_name : '' }}
                                        </a>
                            ...
                            <div class="clearfix" id="galleryddd">
                                ...
                                <ul id="image-gallery" class="gallery list-unstyled cS-hidden">
                                    @if ($recordInfo->youtube_url != '')
                                        <li data-thumb="{{ GIG_FULL_DISPLAY_PATH . $recordInfo->youtube_image }}"
                                            alt="">
                                            <iframe src="{{ App\Models\Gig::get_video_code($recordInfo->youtube_url) }}"
                                                width="580" height="350" frameborder="0" allowfullscreen border="0"
                                                id="player"></iframe>
                                        </li>
                                    ...
                                    @foreach ($recordInfo->Image as $gigimage)
                                        @if (isset($gigimage->name) && !empty($gigimage->name))
                                            ...
                                            @if (file_exists($path) && !empty($gigimage->name))
                                                @php $isanyimage = 1; @endphp
                                                <li data-thumb="{{ GIG_FULL_DISPLAY_PATH . $gigimage->name }}"
                                                    alt="{{ $recordInfo->title }}">
                                        ...
                                        @foreach ($recordInfo->Gigfaq as $key => $gigfaq)
                                            <div class="accordion-section">
                                                <a class="accordion-section-title"
                                                    href="#accordion-{{ $key }}">{{ $gigfaq->question }}</a>
                                ...
                                <div id="four" class="same_box">
                                    ...
                                    <div class="accordion">
                                        ...
                                        @foreach ($recordInfo->Gigextra as $key => $gigextraa)
                                            <div class="accordion-section">
                                                <a class="accordion-section-title"
                                                    href="#accordion-{{ $gigextraa->id }}">{{ $gigextraa->title }}</a>
                                                <div id="accordion-{{ $gigextraa->id }}" class="accordion-section-content">
                                                    <p>{{ $gigextraa->description }}</p>
                                                    <p><strong>Delivery Time :
                                                        </strong>{{ $delivery_days[$gigextraa->delivery] }}</p>
                                                    <input type="hidden" class="extra_price" id="amt"
                                                        value="{{ $gigextraa->price }}">
                                                    <input type="hidden" class="extra_id"
                                                        id="gigextraid<?php echo $gigextraa->price; ?>" value="{{ $gigextraa->id }}">
                                                    <p><strong>Price : </strong>${{ $gigextraa->price }}</p>
                                                    <span class="btn-lrg-standard add_btn" id="addbtn<?php echo $gigextraa->price; ?>"
                                                        onclick="Addamount(this,{{ $gigextraa->id }})"
                                                        style="margin-top: -65px;">ADD</span>
                                                    <span class="js-str-currency" id="btnpricee">

                                                    </span>
                                                    <span class="btn-lrg-standard remv_btn"
                                                        id="removebtn<?php echo $gigextraa->price; ?>"
                                                        onclick="removeAmt(this,{{ $gigextraa->id }})"
                                                        style="margin-top: -65px;display:none;
">Remove</span>
                                            ...
                                            <ul>
                                                @foreach (array_filter($pdf_doc) as $attachmental)
                                                    @if (file_exists(GIG_DOC_FULL_UPLOAD_PATH . $attachmental) && $attachmental != '')
                                                        <li data-img="{{ $attachmental }}" class="portfolio-cc">
                                                            {{ substr($attachmental, 9) }}<a
                                                                href="{{ GIG_DOC_FULL_DISPLAY_PATH . $attachmental }}"
                                                                class="delete" download><i class="fa fa-download"></i></a>
                                                        </li>
                                                    @endif
                                                @endforeach
                                            </ul>
                            ...
                            <div class="about_seller">
                                ...
                                <div class="profile-about">
                                    ...
                                    <div class="dp_details-about">
                                        <h3><a href="{{ URL::to('public-profile/' . $recordInfo->User->slug) }}">{{ $recordInfo->User->first_name ? $recordInfo->User->first_name . ' ' . $recordInfo->User->last_name : '' }}
                                            </a></h3>
                                                ...
                                                <div class="client-chat gigdtl_pgin">
                                                    <div class="clientimg-about">
                                                        @if ($allrecord->Otheruser->profile_image)
                                                            <a href="{{ URL::to('public-profile/' . $allrecord->Otheruser->slug) }}"
                                                                class="">{{ HTML::image(PROFILE_SMALL_DISPLAY_PATH . $allrecord->Otheruser->profile_image, SITE_TITLE) }}</a>
                                                        @else
                                                            <a href="{{ URL::to('public-profile/' . $allrecord->Otheruser->slug) }}"
                                                                class="">{{ HTML::image('public/img/front/user-img.png', SITE_TITLE, ['id' => 'pimage']) }}</a>
                                                        @endif
                                                    </div>
                                                    <div class="client-rv">
                                                        <h3><a href="{{ URL::to('public-profile/' . $allrecord->Otheruser->slug) }}"
                                                                class="">{{ $allrecord->Otheruser->first_name . ' ' . $allrecord->Otheruser->last_name }}</a>
                                                        </h3>
                                                        <span class="review-date"><i class="fa fa-calendar"
                                                                aria-hidden="true"></i>{{ $allrecord->created_at->diffForHumans() }}</span>
                                                        <div class="client-review-reting">
                                                            ...
                                                            <span class="lstreview lstreview_new"
                                                                id="lst{{ $allrecord->id }}"></span>
                        ...
                        <div class="offer_wrap_top offer_wrap_top__new sticky">
                            <div class="offer_wrap_bg__new">
                                ...
                                <ul class="nav offer-nav-tabs" role="tablist">
                                    <li role="presentation"><a href="#basic" class="active" aria-controls="basic"
                                            role="tab" data-toggle="tab"
                                            onclick="updateprice('{{ $recordInfo->basic_price }}', 'basic')">Basic</a>
                                    </li>
                                    <li role="presentation"><a href="#standard" aria-controls="standard" role="tab"
                                            data-toggle="tab"
                                            onclick="updateprice('{{ $recordInfo->standard_price }}', 'standard')">Standard</a>
                                    </li>
                                    <li role="presentation"><a href="#premium" aria-controls="premium" role="tab"
                                            data-toggle="tab"
                                ...
                                <div class="tab-content">
                                        ...
                                        <div class="offer-bxs">
                                            ...
                                            <div class="offers-details">
                                                <span class="offer-icons">
                                                    ...
                                                    <span class="delivery-time">{{ $recordInfo->basic_delivery }}
                                                        days</span>
                                                    Delivery
                                                </span>
                                                <span class="offer-icons">
                                    ...
                                    <div role="tabpanel" class="tab-pane" id="standard">
                                        <div class="offer-bxs">
                                            ...
                                            <div class="offers-details">
                                                <span class="offer-icons">
                                                    <i class="fa fa-clock-o fa-lg"></i>
                                                    <span class="delivery-time">{{ $recordInfo->standard_delivery }}
                                                        days</span>
                                                    Delivery
                                                </span>
                                                <span class="offer-icons">
                                    ...
                                    <div role="tabpanel" class="tab-pane" id="premium">
                                        <div class="offer-bxs">
                                            ...
                                            <div class="offers-details">
                                                <span class="offer-icons">
                                                    <i class="fa fa-clock-o fa-lg"></i>
                                                    <span class="delivery-time">{{ $recordInfo->premium_delivery }}
                                                        days</span>
                                                    Delivery
                                                </span>
                                                <span class="offer-icons">
                                    ...
                                    {{ Form::open(['method' => 'post', 'id' => 'addggiform']) }}
                                    ...
                                    <div class="package-footer">
                                            ...
                                            @if (Session::get('user_id') != $recordInfo->user_id)
                                                ...
                                                @if ($recordInfo->pause == 0 || $recordInfo->User->accept_orders == 0 || $recordInfo->User->hide_weekend == 1)
                                                    ...
                                                    @if (Session::get('user_id'))
                                                        @if (in_array(Session::get('user_id'), $wait))
                                                            <span class="btn-lrg-standard btn-lrg-basc">Added to Waiting
                                                                List</span>
                                                        @else
                                                            <span class="btn-lrg-standard" id="myBtn1"
                                                                onclick="submitwaitlist()" id="waitlist">Add to Waiting
                                                                List</span>
                                                        @endif
                                                    @else
                                                        <span class="btn-lrg-standard" id="myBtn1"
                                                ...
                                                @else
                                                    <span onclick="submitform()" class="btn-lrg-standard">Continue
                                                        ({{ CURR }}<span class="js-str-currency"
                                        ...
                                        <div class="gigdloader" id="gigdloader">
                                            {{ HTML::image('public/img/loading.gif', SITE_TITLE) }}</div>
                                    </div>
                                    {{ Form::close() }}
                                </div>
                            </div>
                            ...
                            @if (Session::get('user_id') != $recordInfo->user_id)
                                <span class="contct_per_detail"><a href="javascript:void(0);" class="btn-lrg-standard"
                                        onclick='$("#info12").show();'data-toggle="modal" data-target="#myModal">Contact
                                        Seller</a></span>
                            @endif
                        </div>
                    </div>
    ...
    <div class="modal fade publicprofile_modal" id="myModal" tabindex="-1" role="dialog"
        ...
        $(function() {
            ...
            minimized_elements.each(function() {
                ...
                $(this).html(
                    ...
                    ' <a href="#" class="less"> - See Less </a></span>'
                );
            });
